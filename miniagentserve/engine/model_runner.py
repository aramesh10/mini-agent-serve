...
class ModelRunner:
    ...
    def __init__(self, 
        ...
        self.last_tokens = torch.zeros(1, dtype=torch.int32, device=self.device)   # the previous step's samples, still on the GPU; none before the first
        ...
        # run eagerly first so flashinfer JIT-builds and caches its kernels: dynamo can't trace the build (it breaks the guards)
        ...
        self.capture_graphs(self.graph_shapes)

    @torch.inference_mode()
    def capture_graphs(self, shapes: list[tuple[int, int]]):
    ...
    @torch.inference_mode()
    def launch(self, seqs: list[Sequence]) -> tuple[torch.Tensor, torch.cuda.Event]:
        """Queues a step on the GPU without waiting for it: returns its samples' host copy and an event marking them ready. Each sequence's sample is at `seq.row`."""
        # Prepare inputs
        ...
        # Forward pass
        self.fill_placeholders(inputs.input_ids)
        if shape:
    ...
    def fill_placeholders(self, input_ids: torch.Tensor):
        """Swaps each placeholder, -1 - r, for row r of the previous step's samples, without the CPU waiting for them."""
        is_placeholder = input_ids < 0
        rows = (-1 - input_ids).clamp(min=0)            # clamped so real tokens index safely too; their lookup is discarded
        sampled = self.last_tokens[rows]
        input_ids.copy_(torch.where(is_placeholder, sampled, input_ids))
    ...
    @staticmethod
    def decode_rows(num_decodes: int) -> int:
        return next((b for b in [0, *GRAPH_BATCH_SIZES] if b >= num_decodes), num_decodes)
    ...
    def prepare(self, decodes: list[Sequence], prefills: list[Sequence], graph_shape: tuple[int, int] | None) -> ModelRunnerGraphInputs:
        ...
        host = torch.empty(size, dtype=torch.uint8, pin_memory=True)    # fresh: the previous step's copy may still be reading the last one
